...
class VoxelMorph2d(nn.Module):
    ...
    def forward(self, moving_image, fixed_image,return_matrix=False):
...
def MSE_loss(I,J):
    #sourceとtargetが類似の画像強度分布と局所コントラストを持つ場合に有効
    I = I.permute(0, 3, 1, 2)
    J = J.permute(0, 3, 1, 2)
    batch_size, channels, xdim, ydim = I.shape
    
    mse = ((I-J)*(I-J)).sum([1,2,3]).mean()/(channels*xdim*ydim)
    return mse
...
def cross_correlation_loss(I, J, n):
    ...
    ave_filter = torch.ones((1, channels, n, n))/(n*n)
    if use_gpu:
        sum_filter = sum_filter.cuda()
        ave_filter = ave_filter.cuda()
    ...
    I_ave = torch.conv2d(I, ave_filter, padding=(n-1)//2, stride=(1,1))
    I_delta = I-I_ave
    I_var = torch.conv2d(I_delta*I_delta,sum_filter,padding=(n-1)//2, stride=(1,1))
    J_ave = torch.conv2d(J, ave_filter, padding=(n-1)//2, stride=(1,1))
    J_delta = J-J_ave
    J_var = torch.conv2d(J_delta*J_delta,sum_filter,padding=(n-1)//2, stride=(1,1))
    cross = torch.conv2d(I_delta*J_delta,sum_filter,padding=(n-1)//2, stride=(1,1))
...
def vox_morph_loss(y, ytrue,deformation_matrix, n=9, lamda=0.01,lamda_MSE=0.1,use_MSE=False,only_MSE=False,print_detail=False):
    mse = MSE_loss(y, ytrue)
    cc = cross_correlation_loss(y, ytrue, n)
    sm = smooothing_loss(deformation_matrix)
    if print_detail:
        print(f'CC Loss {cc}, Gradient Loss {sm} MSE Loss {mse}')
    if use_MSE:
        if only_MSE:
            loss = lamda_MSE * mse + lamda * sm 
        else:
            loss = lamda_MSE * mse + 1.0 -1.0 * cc + lamda * sm     
    else:
        loss = 1.0 -1.0 * cc + lamda * sm 
    return loss
